...
export const providerHandlers = HttpApiBuilder.group(InstanceHttpApi, "provider", (handlers) =>
  Effect.gen(function* () {
    ...
    const list = Effect.fn("ProviderHttpApi.list")(function* () {
      ...
      // THcoder fork: the model picker (this endpoint) merges the full
      // models.dev catalog with connected providers. Default the allow-list
      // to ["tokenharbor"] when the user hasn't set enabled_providers, so
      // catalog providers (OpenCode Zen etc.) don't leak into the picker —
      // matching Provider.list()'s default. Token Harbor isn't in models.dev,
      // so `filtered` ends up empty and only the connected tokenharbor
      // provider (with its full model list) survives the merge below.
