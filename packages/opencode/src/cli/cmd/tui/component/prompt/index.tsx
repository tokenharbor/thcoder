...
export function Prompt(props: PromptProps) {
  ...
  // Red "not signed in" notice shown when a logged-out user tries to send.
  const [authNotice, setAuthNotice] = createSignal<string>()
  ...
  async function submit() {
    ...
    // Signed-out guard (Claude-style): if there's no Token Harbor key in
    // any store, refuse to send and show a red notice prompting /login.
    if (!thReadKey()) {
      setAuthNotice("Not signed in to Token Harbor — run /login to sign in")
      setTimeout(() => setAuthNotice(undefined), 6000)
      return false
    }
    submitting = true
    try {
      ...
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false}>
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
            ...
            <Match when={authNotice()}>
              {(notice) => (
                <box paddingLeft={3}>
                  <text fg={theme.error}>{notice()}</text>
                </box>
              )}
            </Match>
