...
import { thLogin, thLogout } from "@/th-auth"
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const appCommands = createMemo(() =>
    [
      ...
      {
        name: "thcoder.login",
        title: "Sign in to Token Harbor",
        category: "Account",
        slashName: "login",
        run: () => {
          toast.show({ message: "Opening browser to sign in…", variant: "info" })
          dialog.clear()
          void thLogin().then((key) => {
            toast.show(
              key
                ? { message: "Signed in to Token Harbor. Restart thcoder to apply.", variant: "info" }
                : { message: "Sign-in failed or timed out.", variant: "error" },
            )
          })
        },
      },
