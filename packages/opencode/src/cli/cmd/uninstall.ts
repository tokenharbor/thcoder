...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall thcoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall THcoder")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".thcoder") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using THcoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes("# THcoder") ||
      content.includes(".thcoder/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    // Match both the new THcoder installer marker and the legacy opencode one.
    if (trimmed === "# THcoder" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".thcoder/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(".thcoder/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".thcoder") || trimmed.includes(".opencode")))
    ) {
      continue
    }
