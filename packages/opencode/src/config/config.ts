...
import { TH_MODELS } from "@/th-models"
import { TH_COMMANDS, TH_PERMISSION, TH_AGENTS } from "@/th-defaults"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      // completion, but avoid writing when the user explicitly routes
      // config through env-provided paths or content.
      //
      // THcoder fork: seed includes a `tokenharbor` OpenAI-compatible
      // provider pointed at the TH gateway, plus the `th-orchestra`
      // default model. First-run users get the right defaults without
      // any manual config. If they later add their own provider it
      // merges on top in mergeConfig below.
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          const seed = {
            ...
            // THcoder fork: only the Token Harbor provider shows in the
            // model picker. `enabled_providers` is the allow-list the
            // picker honors (see provider.ts isProviderAllowed). The fork
            // also force-defaults this to ["tokenharbor"] for existing
            // configs that predate this seed.
            enabled_providers: ["tokenharbor"],
            provider: {
              tokenharbor: {
                name: "Token Harbor",
                ...
                options: {
                  ...
                  apiKey: "{env:TH_API_KEY}",
                },
                // Full Token Harbor line-up with per-1M cost + context
                // limits (canonical list in src/th-models.ts). th-orchestra
                // (the smart multi-role router) is the default.
                models: TH_MODELS,
              },
            },
            // TH slash commands (prompt templates) + safe tool-permission
            // defaults + the Yolo primary agent. See src/th-defaults.ts.
            command: TH_COMMANDS,
            permission: TH_PERMISSION,
            agent: TH_AGENTS,
          }
      ...
      // THcoder fork: force-merge default slash commands + tool
      // permissions as DEFAULTS (user/project config wins per-key). This
      // applies to existing configs too — the seed above only runs for a
      // brand-new config.json, but these should always be present.
      result.command = { ...TH_COMMANDS, ...(result.command ?? {}) } as typeof result.command
      if (!result.permission) result.permission = TH_PERMISSION as typeof result.permission
      result.agent = { ...TH_AGENTS, ...(result.agent ?? {}) } as typeof result.agent
