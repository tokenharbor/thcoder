  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // THcoder fork: the model picker surfaces ONLY the Token Harbor
        // provider, with its full model line-up. Two enforcements that
        // also apply to configs predating this build:
        //   (1) force-merge the canonical TH model list into the
        //       tokenharbor provider so every model shows even if the
        //       on-disk config.json only lists th-orchestra (older seed).
        //   (2) default enabled_providers to ["tokenharbor"] so other
        //       providers (OpenCode Zen, auth'd providers, models.dev) are
        //       hidden — unless the user EXPLICITLY sets enabled_providers.
        const TH_PROVIDER = "tokenharbor"
        const thExisting: any = (cfg.provider as any)?.[TH_PROVIDER] ?? {}
        const rawProviders: Record<string, any> = { ...(cfg.provider ?? {}) }
        rawProviders[TH_PROVIDER] = {
          name: thExisting.name ?? "Token Harbor",
          npm: thExisting.npm ?? "@ai-sdk/openai-compatible",
          options: {
            baseURL: "https://tokenharbor.ai/v1",
            apiKey: "{env:TH_API_KEY}",
            ...(thExisting.options ?? {}),
          },
          // Full TH catalog with per-1M cost + context limit so the
          // in-session "$ spent" / "% used" gauge is accurate. User
          // overrides (from their config.json) win on a per-model basis.
          models: { ...TH_MODELS, ...(thExisting.models ?? {}) },
        }

        // now read config providers - includes any modifications from plugin config() hook
        const configProviders = Object.entries(rawProviders)
        const disabled = new Set(cfg.disabled_providers ?? [])
        const enabled = cfg.enabled_providers ? new Set(cfg.enabled_providers) : new Set([TH_PROVIDER])
